...
if __name__ == "__main__":
    ...
    while True:
        if os.path.isfile(arguments.input):
            ...
            logging.info(
                f"Extracting archive '{arguments.input}' to temporary directory '{directory}'.")
            ...
            if os.path.isfile(arguments.input + "/model.pnml"):
                logging.info(
                    f"Using directory '{arguments.input}' for input, as it contains a 'model.pnml' file.")
                break
            else:
                logging.error(
                    f"Cannot use directory '{arguments.input}' for input, as it does not contain a 'model.pnml' file.")
                sys.exit(1)
        else:
            logging.error(
                f"Cannot use directory '{arguments.input}' for input, as it does not contain a 'model.pnml' file.")
    ...
    if arguments.tool != None:
        ...
        logging.warning(
            f"Cannot find known information for examination '{examination}' on instance '{instance}' or model '{model}'.")
